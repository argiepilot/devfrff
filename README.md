...
**Technology:** Python with [uv](https://docs.astral.sh/uv/) for environment and dependency management.
...
After the packages are installed in ForeFlight, you can delete the generated folders to reclaim disk space:
...
- [uv](https://docs.astral.sh/uv/getting-started/installation/) (installs Python 3.11 automatically)
...
uv run python run.py process-all
...
# Limit FAA charts
uv run python run.py process-all --limit-faa 2

# Verbose output for debugging
uv run python run.py process-all --verbose

# After importing the packages into ForeFlight, free disk space
uv run python run.py clean
uv run python run.py clean --yes
uv run python run.py clean --dry-run
uv run python run.py clean "My Charts"
```
...
uv run python run.py process-faa-terminal --limit 1
uv run python run.py process-faa-terminal --quick --limit 1

# FAA Sectional charts
uv run python run.py process-faa-sectional --limit 1
uv run python run.py process-faa-sectional --quick --limit 1

# Unified package, but skip DFS while iterating on FAA
uv run python run.py process-all --no-dfs --faa-quick --limit-faa 1
```

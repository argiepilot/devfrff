...
def clean(
    directories: Optional[List[str]] = typer.Argument(
        None,
        help=(
            "Package directories to remove. Defaults to "
    ...
    table = Table(title="Directories to remove")
    ...
    if not yes:
        if not sys.stdin.isatty():
            ...
            raise typer.Exit(1)
        if not Confirm.ask(
            "Delete these directories? This cannot be undone.", default=False
        ):
